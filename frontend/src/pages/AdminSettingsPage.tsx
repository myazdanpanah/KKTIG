...
export default function AdminSettingsPage() {
  ...
  const filteredTeams = teams.filter((ft) => !form.division || ft.division === form.division)
    ...
    try {
      if (editingUser) {
        ...
        if (!form.password || form.password.length < 8) {
          toast(t('adminFormPasswordMin'), 'error')
          return
        }
  ...
  const handleDelete = async (u: User) => {
    ...
    if (!window.confirm(`${t('adminDeleteUserConfirm')} «${u.username}» ${t('adminDeleteUserConfirm2')}`)) return

    try {
      await api.delete(`/auth/users/${u.id}/`)
      toast(t('adminUserDeleted'), 'success')
      fetchUsers()
    } catch {
      toast(t('adminFormSaveError'), 'error')
    }
  }

  const handleRoleSubmit = async () => {
    if (!editingRole.value || !editingRole.label) {
      toast(t('adminRoleRequired'), 'error')
      return
    }
    try {
      if (editingRole.id) {
        ...
        toast(t('adminRoleUpdated'), 'success')
      } else {
        ...
        toast(t('adminRoleCreated'), 'success')
      }
      ...
      const axiosErr = err as { response?: { data?: { error?: string } } }
      toast(axiosErr.response?.data?.error || t('adminRoleSaveError'), 'error')
    }
  }

  const handleDeleteRole = async (roleId: number) => {
    if (!window.confirm(t('adminRoleDeleteConfirm'))) return
    try {
      await api.delete(`/auth/roles/${roleId}/`)
      toast(t('adminRoleDeleted'), 'success')
      fetchCustomRoles()
    } catch {
      toast(t('adminRoleDeleteError'), 'error')
    }
  }
  ...
  const handleClearAll = async () => {
    if (!window.confirm(`⚠️ ${t('adminClearConfirm1')}`)) return
    if (!window.confirm(`⚠️ ${t('adminClearConfirm2')}`)) return
    setClearing(true)
    try {
      await api.delete('/dashboards/clear-all/', { data: { confirm: true } })
      toast(t('adminAllDataDeleted'), 'success')
    } catch (err: unknown) {
      const axiosErr = err as { response?: { data?: { error?: string } } }
      toast(axiosErr.response?.data?.error || t('adminFormSaveError'), 'error')
    } finally {
  ...
  const ACTION_LABELS: Record<string, string> = {
    dashboard_share: t('auditDashboardShare'),
    dashboard_create: t('auditDashboardCreate'),
    dashboard_delete: t('auditDashboardDelete'),
    page_access_update: t('auditPageAccessUpdate'),
    filter_access_update: t('auditFilterAccessUpdate'),
    user_role_change: t('auditUserRoleChange'),
  }
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-bold text-gray-900 dark:text-gray-100">{t('adminTitle')}</h1>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        {user?.isStaff && (
          <div className="mb-8 bg-white dark:bg-gray-800 rounded-2xl border-2 border-red-200 dark:border-red-800 overflow-hidden">
            <div className="px-6 py-4 border-b border-red-100 flex items-center justify-between">
              <div>
                <h2 className="font-bold text-red-700 dark:text-red-400">{t('adminDangerZone')}</h2>
                <p className="text-xs text-red-400">{t('adminDangerDesc')}</p>
              </div>
              <button
              ...
              >
                ...
                {clearing ? t('adminClearing') : t('adminClearAll')}
              </button>
            </div>
          </div>
        )}
        ...
        <div className="flex items-center gap-1 mb-6">
          <button onClick={() => setActiveTab('users')} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition ${activeTab === 'users' ? 'bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>
            <Users className="w-4 h-4" /> {t('adminTabUsers')}
          </button>
          <button onClick={() => setActiveTab('audit')} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition ${activeTab === 'audit' ? 'bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>
            <History className="w-4 h-4" /> {t('adminTabAudit')}
          </button>
          <button onClick={() => setActiveTab('roles')} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition ${activeTab === 'roles' ? 'bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>
            <Tag className="w-4 h-4" /> {t('adminTabRoles')}
          </button>
        </div>
        ...
        {activeTab === 'users' && (
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-700">
              <h2 className="font-bold text-gray-900 dark:text-gray-100">{t('adminUserList')} ({users.length})</h2>
            </div>
            {loading ? (
                ...
                <table className="w-full">
                  <thead>
                    <tr className="text-right text-xs text-gray-500 dark:text-gray-400 border-b border-gray-100 dark:border-gray-700">
                      <th className="px-6 py-3 font-medium">{t('adminColUser')}</th>
                      <th className="px-6 py-3 font-medium">{t('adminColEmail')}</th>
                      <th className="px-6 py-3 font-medium">{t('adminColRole')}</th>
                      <th className="px-6 py-3 font-medium">{t('adminColOrg')}</th>
                      <th className="px-6 py-3 font-medium">{t('adminColActions')}</th>
                    </tr>
                  </thead>
                        ...
                        <tr key={u.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-1">
                              <button onClick={() => openEditModal(u)} className="p-1.5 text-gray-400 hover:text-indigo-500 hover:bg-indigo-50 rounded-lg transition" title={t('adminEdit')}><Pencil className="w-4 h-4" /></button>
                              {u.id !== user?.id && (
                                <button onClick={() => handleDelete(u)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition" title={t('adminDelete')}><Trash2 className="w-4 h-4" /></button>
                              )}
                            </div>
                          </td>
                        </tr>
        ...
        {activeTab === 'audit' && (
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-700">
              <h2 className="font-bold text-gray-900 dark:text-gray-100">{t('adminAuditTitle')} ({auditLogs.length})</h2>
            </div>
            {auditLoading ? (
              <div className="p-12 text-center text-gray-500">{t('loading')}</div>
            ) : auditLogs.length === 0 ? (
              <div className="p-12 text-center">
                ...
                <p className="text-gray-500">{t('adminAuditNoLogs')}</p>
              </div>
            ) : (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-400">
                            {t('adminAuditBy')} {log.user || t('adminAuditSystem')} · {formatDateTime(log.created_at)}
                          </p>
                        </div>
                      </div>
                    ...
                    {expandedLog === log.id && (
                      <div className="mt-3 pl-6 border-r-2 border-indigo-200 pr-3 space-y-2">
                        {Object.keys(log.old_value).length > 0 && (
                          <div>
                            <span className="text-[10px] text-gray-400 uppercase tracking-wide">{t('adminAuditBefore')}</span>
                        ...
                        {Object.keys(log.new_value).length > 0 && (
                          <div>
                            <span className="text-[10px] text-gray-400 uppercase tracking-wide">{t('adminAuditAfter')}</span>
                        ...
                        {Object.keys(log.details).length > 0 && (
                          <div>
                            <span className="text-[10px] text-gray-400 uppercase tracking-wide">{t('adminAuditDetails')}</span>
        ...
        {activeTab === 'roles' && (
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
              <h2 className="font-bold text-gray-900 dark:text-gray-100">{t('adminTabRoles')}</h2>
              <button
              ...
              >
                <Plus className="w-3.5 h-3.5" /> {t('adminRoleNew')}
              </button>
            </div>
            <div className="p-6 space-y-4">
              <div>
                <h3 className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3">{t('adminRolesBuiltIn')}</h3>
              ...
              <div>
                <h3 className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3">{t('adminRolesCustom')}</h3>
                {customRoles.length === 0 ? (
                  <div className="text-center py-8">
                    <Tag className="w-10 h-10 text-gray-300 mx-auto mb-2" />
                    <p className="text-sm text-gray-500">{t('adminRolesCustomEmpty')}</p>
                    <button
                      onClick={() => { setEditingRole({ value: '', label: '', color: 'bg-gray-100 text-gray-700' }); setShowRoleModal(true) }}
                      className="mt-2 text-xs text-indigo-600 hover:text-indigo-800 font-medium"
                    >{t('adminRolesCustomCreate')}</button>
                  </div>
                ) : (
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {customRoles.map((r) => (
                      <div key={r.id} className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl border border-gray-200 dark:border-gray-600 group">
                        ...
                        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition">
                          <button onClick={() => { setEditingRole(r); setShowRoleModal(true) }} className="p-1 text-gray-400 hover:text-indigo-500 rounded transition"><Pencil className="w-3 h-3" /></button>
                          <button onClick={() => handleDeleteRole(r.id)} className="p-1 text-gray-400 hover:text-red-500 rounded transition"><Trash2 className="w-3 h-3" /></button>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md mx-4">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700">
              <h3 className="font-bold text-gray-900 dark:text-gray-100">{editingRole.id ? t('adminRoleEdit') : t('adminRoleNew')}</h3>
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">{t('adminRoleId')}</label>
                <input type="text" value={editingRole.value} onChange={(e) => setEditingRole({ ...editingRole, value: e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, '') })} className="w-full px-3 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder={t('adminRoleIdPlaceholder')} disabled={!!editingRole.id} />
                <p className="text-[10px] text-gray-400 mt-1">{t('adminRoleIdHint')}</p>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">{t('adminRoleLabel')}</label>
                <input type="text" value={editingRole.label} onChange={(e) => setEditingRole({ ...editingRole, label: e.target.value })} className="w-full px-3 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder={t('adminRoleLabelPlaceholder')} />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">{t('adminRoleColor')}</label>
                <div className="flex flex-wrap gap-2">
                  {COLOR_OPTIONS.map((c) => (
                    <button key={c} onClick={() => setEditingRole({ ...editingRole, color: c })} className={`w-8 h-8 rounded-lg ${c} flex items-center justify-center transition ring-2 ${editingRole.color === c ? 'ring-indigo-500 ring-offset-2' : 'ring-transparent'}`}>
              ...
              <div className="p-3 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl">
                <p className="text-xs text-indigo-600 dark:text-indigo-400 font-medium">{t('adminRolePreview')}</p>
                <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium mt-1 ${editingRole.color}`}>
                  <Tag className="w-3.5 h-3.5" />
                  {editingRole.label || t('adminRolePlaceholder')}
                </span>
              </div>
            </div>
            <div className="px-6 py-4 border-t border-gray-100 dark:border-gray-700 flex items-center justify-end gap-3">
              <button onClick={() => setShowRoleModal(false)} className="px-4 py-2 text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-xl transition">{t('cancel')}</button>
              <button onClick={handleRoleSubmit} className="px-6 py-2 bg-indigo-600 text-white text-sm rounded-xl hover:bg-indigo-700 transition font-medium">
                {editingRole.id ? t('adminRoleSave') : t('adminRoleCreate')}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700 sticky top-0 bg-white dark:bg-gray-800 z-10">
              <h3 className="font-bold text-gray-900 dark:text-gray-100">{editingUser ? t('adminEditUser') : t('adminNewUser')}</h3>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">{t('adminFormFirstName')}</label>
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">{t('adminFormLastName')}</label>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">{t('adminFormUsername')}</label>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">{t('adminFormEmail')}</label>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">{t('adminFormRole')}</label>
                {(() => {
                  ...
                  return (
                    <div className="relative" ref={roleDropdownRef}>
                      <button type="button" onClick={() => setOpenRoleDropdown(!openRoleDropdown)} className="w-full flex items-center justify-between gap-2 px-3 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 text-sm hover:border-indigo-400 transition focus:ring-2 focus:ring-indigo-500 outline-none">
                        <div className="flex items-center gap-2">
                          {selectedRole ? (
                            ...
                            <span className="text-gray-400">{t('adminFormRole')}</span>
                          )}
                        </div>
                      ...
                      {openRoleDropdown && (
                        <div className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-xl shadow-xl z-50 p-3 max-h-60 overflow-y-auto">
                          <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-2">{t('adminRolesBuiltIn')}</p>
                          <div className="grid grid-cols-2 gap-1.5 mb-3">
                            {ROLE_OPTIONS.map((r) => (
                              <button key={r.value} type="button" onClick={() => { setForm({ ...form, role: r.value }); setOpenRoleDropdown(false) }} className={`flex items-center gap-2 p-2 rounded-lg text-sm font-medium transition ${form.role === r.value ? 'bg-indigo-50 dark:bg-indigo-900/30 ring-2 ring-indigo-500 text-indigo-700 dark:text-indigo-300' : 'hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-400'}`}>
                                <span className={`w-6 h-6 rounded-md flex items-center justify-center ${r.color}`}><Shield className="w-3 h-3" /></span>
                                <span>{r.label}</span>
                              </button>
                            ))}
                          </div>
                          {customRoles.length > 0 && (
                            <>
                              <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-2">{t('adminRolesCustom')}</p>
                              <div className="grid grid-cols-2 gap-1.5">
                                {customRoles.map((cr) => (
                                  <button key={cr.value} type="button" onClick={() => { setForm({ ...form, role: cr.value }); setOpenRoleDropdown(false) }} className={`flex items-center gap-2 p-2 rounded-lg text-sm font-medium transition ${form.role === cr.value ? 'bg-indigo-50 dark:bg-indigo-900/30 ring-2 ring-indigo-500 text-indigo-700 dark:text-indigo-300' : 'hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-400'}`}>
                                    <span className={`w-6 h-6 rounded-md flex items-center justify-center ${cr.color}`}><Tag className="w-3 h-3" /></span>
                                    <span>{cr.label}</span>
                                  </button>
                                ))}
                              </div>
              ...
              <div className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl space-y-3">
                <p className="text-xs font-bold text-gray-700 dark:text-gray-300">{t('adminFormOrgTitle')}</p>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[10px] font-medium text-gray-500 dark:text-gray-400 mb-1">{t('adminFormCompany')}</label>
                    <select value={form.company || ''} onChange={(e) => setForm({ ...form, company: Number(e.target.value) || null, division: null, team: null })} className="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
                      <option value="">{t('adminFormNoCompany')}</option>
                      {companies.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-[10px] font-medium text-gray-500 dark:text-gray-400 mb-1">{t('adminFormDivision')}</label>
                    <select value={form.division || ''} onChange={(e) => setForm({ ...form, division: Number(e.target.value) || null, team: null })} className="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" disabled={!form.company}>
                      <option value="">{t('adminFormNoDivision')}</option>
                      {filteredDivisions.map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-[10px] font-medium text-gray-500 dark:text-gray-400 mb-1">{t('adminFormTeam')}</label>
                    <select value={form.team || ''} onChange={(e) => setForm({ ...form, team: Number(e.target.value) || null })} className="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" disabled={!form.division}>
                      <option value="">{t('adminFormNoTeam')}</option>
                      {filteredTeams.map((ft) => <option key={ft.id} value={ft.id}>{ft.name}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-[10px] font-medium text-gray-500 dark:text-gray-400 mb-1">{t('adminFormReportsTo')}</label>
              ...
              {!editingUser && (
                <div>
                  <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">{t('adminFormPassword')}</label>
                  <input type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} className="w-full px-3 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none" placeholder={t('adminFormPasswordPlaceholder')} />
                </div>
              )}
            </div>

            <div className="px-6 py-4 border-t border-gray-100 dark:border-gray-700 flex items-center justify-end gap-3">
              <button onClick={() => setShowModal(false)} className="px-4 py-2 text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-xl transition">{t('adminFormCancel')}</button>
              <button onClick={handleSubmit} className="px-6 py-2 bg-indigo-600 text-white text-sm rounded-xl hover:bg-indigo-700 transition font-medium">
                {editingUser ? t('adminFormSaveChanges') : t('adminFormCreateUser')}
              </button>
            </div>
          </div>
        </div>
